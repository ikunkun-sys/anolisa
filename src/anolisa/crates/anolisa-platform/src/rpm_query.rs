...
mod tests {
    ...
    impl CommandRunner for FakeCommandRunner {
        fn run(&self, program: &str, args: &[&str]) -> io::Result<CommandOutput> {
            assert_call_contract(program, args);
    ...
    /// subcommand, or omits the package argument would still pass the
    /// output-based assertions.
    fn assert_call_contract(program: &str, args: &[&str]) {
        match program {
            RPM => {
                assert_eq!(
                    args.len(),
    ...
    fn query_with_rpm(outcome: FakeOutcome) -> RpmPackageQuery<FakeCommandRunner> {
    ...
    fn query_with_dnf(outcome: FakeOutcome) -> RpmPackageQuery<FakeCommandRunner> {
        RpmPackageQuery::with_runner(FakeCommandRunner {
            rpm: None,
            dnf: Some(outcome),
        })
    ...
    fn installed_returns_info() {
        let q = query_with_rpm(ok_out(
            Some(0),
            "anolisa-tokenless|(none)|2.0.1|1.al8|x86_64",
            "",
        ));
        let info = q
    ...
    fn not_installed_returns_none() {
        let q = query_with_rpm(ok_out(
            Some(1),
            "package anolisa-tokenless is not installed",
            "",
        ));
    ...
    fn command_missing_maps_to_error() {
        let q = query_with_rpm(FakeOutcome::Err(io::ErrorKind::NotFound));
    ...
    fn permission_denied_maps_to_error() {
        let q = query_with_rpm(FakeOutcome::Err(io::ErrorKind::PermissionDenied));
        let err = q.query_installed("x").unwrap_err();
        assert!(matches!(
            err,
    ...
    fn query_failure_maps_to_error() {
        ...
        let q = query_with_rpm(ok_out(Some(1), "", "error: rpmdb open failed"));
        let err = q.query_installed("x").unwrap_err();
        match err {
    ...
    fn unexpected_field_count_maps_to_error() {
        let q = query_with_rpm(ok_out(Some(0), "anolisa-tokenless|2.0.1", ""));
    ...
    fn multiple_installed_is_unexpected() {
        ...
        let q = query_with_rpm(ok_out(Some(0), two, ""));
        let err = q.query_installed("anolisa-tokenless").unwrap_err();
        match err {
    ...
    fn epoch_none_normalizes() {
        let q = query_with_rpm(ok_out(Some(0), "pkg|(none)|2.3|4|x86_64", ""));
    ...
    fn epoch_set_renders_evr() {
        let q = query_with_rpm(ok_out(Some(0), "pkg|1|2.3|4|x86_64", ""));
        let info = q.query_installed("pkg").unwrap().unwrap();
    ...
    fn epoch_zero_normalizes_like_none() {
        ...
        let q = query_with_rpm(ok_out(Some(0), "pkg|0|2.3|4|x86_64", ""));
        let info = q.query_installed("pkg").unwrap().unwrap();
        assert_eq!(info.version.epoch, None);
        assert_eq!(info.version.to_string(), "2.3-4");
    }
    ...
    fn available_returns_candidates() {
        ...
        let q = query_with_dnf(ok_out(Some(0), out, ""));
    ...
    fn available_epoch_zero_normalizes() {
        ...
        let q = query_with_dnf(ok_out(Some(0), "pkg|0|2.3|4|x86_64|anolisa\n", ""));
    ...
    fn available_empty_returns_empty_vec() {
        let q = query_with_dnf(ok_out(Some(0), "", ""));
    ...
    fn available_failure_maps_to_error() {
        let q = query_with_dnf(ok_out(Some(1), "", "error: repo not found"));
    ...
    fn available_bad_fields_maps_to_error() {
        let q = query_with_dnf(ok_out(Some(0), "pkg|2.0.1\n", ""));
